...
// slct3 will be used for the ogranic checkbox additional filter
...
function populateListProductChoices(slct1, slct2, slct3) {
	...
	for (i = 0; i < optionArray.length; i++) {
		...
		var productName = optionArray[i][1];
		var productPrice = optionArray[i][0];
...
function selectedItems() {
	...
	c.appendChild(para);
	var priceCalc = getTotalPrice(chosenProducts).toFixed(2);
	c.appendChild(document.createTextNode("Total Price is $" + priceCalc));
}

function accordionPress(accordionInput) {
	var acc = document.getElementById(accordionInput);
